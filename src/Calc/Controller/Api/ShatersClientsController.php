...
class ShatersClientsController extends BaseController
{
    const MODAL_ID = '#shaters_client_modal';
    const FORM_ID = '#client_form';

    /**
     * @var string
     */
    protected $repositoryClassName = 'Calc\Repo\ShatersClientRepo';
    /**
    ...
    {
        return $this->response->data([
            'modal' => view('calc::shaters.clients.modal')->with(
    ...
    {
		$obj = ShatersClient::with([
			'files'
		])->findOrFail($id);

        return $this->response->data([
            'modal' => view('calc::shaters.clients.modal')->with('obj', $obj)->render(),
            'modal_id' => self::MODAL_ID,
        ]);
    }
