...
use Calc\Repo\ShatersCalculationRepo;
...
/**
 * Class ShatersMakeOrderController
 */
class ShatersMakeOrderController extends BaseController
{
    protected $repositoryClassName = 'Calc\Repo\ShatersCalculationRepo';

    /**
     * @var ShatersCalculationRepo
     */
    ...
    public function show($id)
    {
        ...
        return view('calc::shaters.orders.order', $data);
    }
    ...
    public function contract($id)
    {
        ...
        /** @var ShatersCalculationWrapper $costs */
        $costs = $data['costs'];
...
//        $word = new TemplateProcessor('https://drive.google.com/uc?export=download&id=1zZe5aMvkyCMik9NjrfKPmH6UKfN4-ZHy');
		$word = new TemplateProcessor(app_path() . '/docrender/shatersContractCalc.docx');
    ...
    public function check($id)
    {
        ...
        /** @var ShatersSubjectWrapper[] $subjects */
        ...
        $objPHPExcel = PHPExcel_IOFactory::load(app_path() . '/docrender/shatersCheck.xls');
...
//            'CLIENT_SECONDNAME' => $order->client->second_name,
    ...
    public function spec($id)
    {
        ...
        $word = new TemplateProcessor(app_path() . '/docrender/shatersSpec.docx');
//        $word = new TemplateProcessor(app_path() . '/docrender/shatersSpec.docx');
        ...
        $titleWidth = Converter::pixelToTwip(460);
		...
		/*$table
			...
			->addText('Проектирование', $headTextStyle);*/
		/*$table
			->addCell(
				$titleWidth,
				array_merge(
			...
			->addText('Стоимость', $headTextStyle);*/
        ...
        if ($order->pseudo_discount_percent) {
            $table
                ->addCell($priceWidth, array_merge($headCellStyle, ['width' => $titleWidth]))
//                ->addText("Со скидкой ({$order->pseudo_discount_percent}%)", $headTextStyle);
        ...
        foreach ($subjects as $subject) {
			...
			*/
            $table
				...
				->addText($subject->title, $priceTextStyle);
//				->addText($subject->title . '<w:br />' . "({$subject->x} × {$subject->y} × {$subject->z} мм)", $priceTextStyle);
...
//            'DELIVERY_DAY' => '__',
        ...
        $word = new TemplateProcessor(app_path() . '/docrender/shatersAcceptance.docx');
    ...
    public function work($id)
    {
        ...
        /** @var ShatersOrder $order */
        $ordr = $order->orders->first();

        if($ordr && $ordr instanceof ShatersOrder) {
...
//            'CLIENT_SECONDNAME' => $order->client->second_name,
        ...
        $word = new TemplateProcessor(app_path() . '/docrender/shatersWork.docx');
...
//			'CLIENT_SECONDNAME' => $order->client->second_name,
		...
		$word = new TemplateProcessor(app_path() . '/docrender/shatersAddagree.docx');
