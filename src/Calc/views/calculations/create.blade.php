...
<h1>
  ...
  @elseif($obj->exists && $obj->child_id)
    ...
    <a class="btn btn-primary btn-md pull-right" href="/calculation/{{$obj->child_id}}/edit">Редактировать Доп.соглашение</a>
  @elseif($obj->exists)
    @lang('calc::titles.calculation_edit')
    <a class="btn btn-primary btn-md pull-right" href="/calculation/{{$obj->id}}/addagree">Создать Доп.соглашение</a>
  @else
      ...
      <div style="margin: 40px 0">
        ...
        @if($obj->parent_id)
          <div class="row calc-docs-generate">
            <div class="col-sm-9">
              <a target="_blank" href="/calculation/addagree/{{ $obj->id }}" ng-show="calc.model.reportable" class="btn btn-info btn-lg">Генерация Доп.соглашения</a>
              <a target="_blank" href="/calculation/check/{{ $obj->id }}" ng-show="calc.model.reportable" class="btn btn-info btn-lg">Генерация счёта</a>
            </div>
          </div>
        @elseif ($obj->exists)
          <div class="row calc-docs-generate">
            <div class="col-sm-9">
                <a target="_blank" href="/calculation/contract/{{ $obj->id }}" ng-show="calc.model.reportable" class="btn btn-info btn-lg">Генерация договора</a>
                <a target="_blank" href="/calculation/spec/{{ $obj->id }}" ng-show="calc.model.reportable" class="btn btn-info btn-lg">Генерация спецификации</a>
                <a target="_blank" href="/calculation/check/{{ $obj->id }}" ng-show="calc.model.reportable" class="btn btn-info btn-lg">Генерация счёта</a>
            </div>
          </div>
          ...
          @if ($obj->status == 7)
            <div class="row calc-docs-generate">
              <div class="col-sm-12">
                <a target="_blank" href="/calculation/certificate/{{ $obj->id }}" class="btn btn-info btn-lg">Генерация сертификата</a>
                <a target="_blank" href="/calculation/acceptance/{{ $obj->id }}" class="btn btn-info btn-lg">Генерация акта приёма-передачи</a>
                <a target="_blank" href="/calculation/work/{{ $obj->id }}" class="btn btn-info btn-lg">Генерация акта оказания работ</a>
              </div>
            </div>
          @endif
        @endif
